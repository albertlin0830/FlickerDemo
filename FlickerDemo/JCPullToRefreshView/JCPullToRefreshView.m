...
#define kImageViewAnimationKey @"refreshAnimation"
...
- (instancetype)initWithScrollView:(UIScrollView *)scrollView
{
    if (self = [super init]) {
        ...
        CGFloat width = 40.0f;
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width-width)/2, (kDefaultDistance-width)/2, width, width)];
...
- (void)startAnimation
{
    ...
    [self.imageView.layer addAnimation:animation forKey:kImageViewAnimationKey];
}

- (void)stopAnimation
{
    [self.imageView.layer removeAnimationForKey:kImageViewAnimationKey];
}
